...
class WSManagerClass {
     ...
     */
    addWsClient = ({ name, api, listener = null, delayMessage = this.defaultDelayMessageTimeMs, ...rest }) => {
        if (name && api) {
            this.wsClientList = {
                ...this.wsClientList,
                [name]: {
                ...
                if (this.ws[id] && listener) {
                    ...
                    this.ws[id].onClose = () => {
                        if (getAppMode() !== 'production') {
                            console.info(`ws [${id}] was closed, reconnecting in 2 seconds %c@ ${new Date().toUTCString()}`, 'color: #255897;')
                        }
                        ...
                        if (reconnectAttempts < this.maxReconnectAttempts) {
                            ...
                            console.error(`ws [${id}] was opened %c@ ${new Date().toUTCString()}`, 'color: #255897;')
                        }
                    }
