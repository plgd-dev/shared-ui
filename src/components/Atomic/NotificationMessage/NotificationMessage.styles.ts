...
export const messageInner = (theme: ThemeType) => css`
    ...
    max-width: 715px;
    margin: 0 auto;
    background: ${getThemeColor(theme, `NotificationMessage.message.background`)};
...
export const type = (type: ToastTypesType, theme: ThemeType) => css`
    color: ${getColorByType(type)};
    padding-left: 12px;
...
export const messageText = (theme: ThemeType) => css`
    font-family: ${getTheme(theme, `Global.fontPrimary`, fontPrimary)};
    font-style: normal;
    font-weight: 400;
    font-size: 16px;
